...
const VERTEX_AI_PROXY_URL = '/vertex-ai-proxy';
...
export const getMenuRecommendation = async (query: string, menu: MenuItem[]): Promise<string> => {
  try {
    ...
    const prompt = `Given the following menu:\n${menuList}\n\nCustomer query: "${query}". Recommend a menu item and explain why it's a good fit. Please format your response using Markdown. Use **bold** for the dish name and use bullet points for the explanation.`;
    const response = await axios.post(VERTEX_AI_PROXY_URL, { prompt });
    return response.data.text;
  } catch (error) {
...
export const refineMenuItemDescription = async (item: MenuItem): Promise<string> => {
  try {
    const prompt = `Refine the description for the following menu item, making it more enticing and descriptive, focusing on its unique flavors and ingredients. The item is: "${item.name}" (${item.category}) with current description: "${item.description}".`;
    const response = await axios.post(VERTEX_AI_PROXY_URL, { prompt });
    return response.data.text;
  } catch (error) {
