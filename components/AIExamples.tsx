...
const AIExamples: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const displayImage = (container: HTMLElement, base64: string) => {
            container.innerHTML = `<img src="${base64}" alt="Food photo" class="rounded-lg object-cover w-full h-full">`;
        };

        const showLoading = (container: HTMLElement, text: string) => {
            container.innerHTML = `<div class="flex flex-col items-center justify-center h-full"><p class="mt-2 text-text-secondary text-sm">${text}</p></div>`;
        };

        const showError = (container: HTMLElement, text: string) => {
        ...
        const handleEnhanceClick = async () => {
            ...
            const payload = { prompt: prompt, image: { mimeType: imageMimeType, data: pureBase64 } };
